...
/** @internal */
final class ProjectAccess implements ProjectAccessInterface
{
    ...
    {
        $active = $this->pdo->prepare('SELECT active FROM users WHERE id=?');
        $active->execute([$user]);
         ...
         */
        $permissions = rbac()->assignments->permissionsOf($user, Scope::of(Project::SCOPE, $id));
    ...
    {
        if (!in_array(Installation::ADMIN_PROJECTS, rbac()->assignments->permissionsOf($user), true)) {
    ...
    /**
     * Read the configured grants of a role, not the effective grants of a user.
     *
    ...
    public function permissions(int $project, string $role, ?int $customRoleId = null): array
    {
        if ($customRoleId === null) {
            ...
            return $id === null ? [] : (rbac()->roles->find($id)['permissions'] ?? []);
        }
