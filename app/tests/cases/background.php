...
$store       = new AttachmentStorage(
    new Storage(new LocalAdapter($privateRoot)),
    $privateRoot,
);
$c->set(AttachmentStorage::class, $store);
$attachments = $c->make(AttachmentService::class);
$upload      = fn(string $body, string $name = 'note.txt') => new UploadedFile(
    Stream::create($body),
    strlen($body),
    UPLOAD_ERR_OK,
    $name,
...
test('private attachment upload download and delete', function () use (
    $attachments,
    $a,
    $id,
    $upload,
    ...
    check(
        stream_get_contents($record['stream']) === 'Private attachment test.',
        'wrong file bytes',
    );
    ...
    denied(404, fn() => $attachments->download($a, $id, $file));
    check(
        (int) scalar('SELECT COUNT(*) FROM attachments') === 0,
        'attachment metadata survived delete',
    );
});
test('upload type size path and orphan boundaries', function () use ($store, $upload) {
    foreach (
        [
            fn() => $store->stage($upload('not a png', 'evil.png')),
            fn() => $store->stage($upload('<?php echo 1;', 'evil.php')),
            fn() => $store->stage($upload(str_repeat('a', 100)), 10),
            fn() => $store->open('../secret'),
        ] as $fn
    ) {
        try {
            $fn();
...
test('PDO queue rollback recovery lease fencing and deadletter', function () use ($pdo) {
    ...
    try {
        ...
        check(str_contains($exception->getMessage(), 'lease was lost'), 'wrong stale error');
    }
...
test('notification visibility rechecks membership and preferences', function () use (
    $auth,
    $users,
    $c,
    $projects,
    ...
    $notifications = $c->make(NotificationService::class);
