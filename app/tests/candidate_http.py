...
status,body,_=request('/login',urllib.parse.urlencode({'_csrf':token,'email':'alice@example.test','password':'Nafinity-Demo-2026!'}).encode())
assert status==200 and b'Ein guter Start' in body, ('authenticated board',status,body[:150])
checked=[]
for path in ['/projects/1','/projects/1/tickets/9','/projects/1/settings','/notifications','/preferences']:
...
assert status==200 and headers['Content-Disposition'].startswith('attachment;'), 'private download failed'
print(json.dumps({'mode':'unreleased-source-snapshot','schema':'202609140003','login':'passed','authenticated_pages':checked,'foreign_project_status':404,'private_download_bytes':len(body),'private_download_sha256':hashlib.sha256(body).hexdigest()},indent=2))
