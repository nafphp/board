...
if (getenv('APP_ENV') !== 'test' || getenv('DB_DATABASE') !== 'nafinity_test') {
    fwrite(STDERR, "Use APP_ENV=test DB_DATABASE=nafinity_test. Demo data is never a test target.\n");
    exit(2);
}
...
try {
    ...
    foreach (['alice','bob','viewer','member'] as $name) {
        $user = new User(['name' => ucfirst($name),'email' => $name.'@example.test','password_hash' => password_hash('Test-Password-2026!', PASSWORD_DEFAULT),'created_at' => gmdate('Y-m-d H:i:s')]);
        $em->save($user);
        $users[$name] = $user;
    }
    ...
    $data = ['title' => 'Searchable sunflower','description' => 'Detailed work item','priority' => 'normal','column_id' => $col,'swimlane_id' => $lane];
    ...
    test('ORM persisted ticket and transactional activity', function () use ($pdo, $id, $query, $a) {
        check($query->detail($a, $id)['ticket']['title'] === 'Searchable sunflower', 'ticket missing');
        check((int)scalar("SELECT COUNT(*) FROM activities WHERE ticket_id=? AND event_type='ticket.created'", [$id]) === 1, 'activity missing');
    });
    test('cross-project column/lane/assignee/label reject with rollback', function () use ($tickets, $a, $data, $current, $boardB, $users, $labelB) {
        $before = scalar('SELECT COUNT(*) FROM tickets');
        $activity = scalar('SELECT COUNT(*) FROM activities');
        foreach (['column_id' => $boardB['columns'][0]['id'],'swimlane_id' => $boardB['swimlanes'][0]['id'],'assignee_ids' => [$users['bob']->getId()],'label_ids' => [$labelB]] as $key => $value) {
            denied(422, fn () => $tickets->create($a, array_replace($data, $current(), [$key => $value])));
    ...
    test('payload types and invalid date rejected', function () use ($tickets, $a, $data, $current) {
        foreach (['title' => [], 'priority' => [], 'due_date' => '2026-02-30','assignee_ids' => '1,2','label_ids' => [[]]] as $key => $value) {
            denied(422, fn () => $tickets->create($a, array_replace($data, $current(), [$key => $value])));
        }
    });
    test('viewer writes denied', function () use ($auth, $users, $tickets, $a, $data, $current, $projects, $comments, $id) {
        $auth->setIdentity($users['viewer']);
        foreach ([fn () => $tickets->create($a, $data + $current()),fn () => $tickets->move($a, $id, []),fn () => $projects->archive($a, true),fn () => $comments->save($a, $id, ['body' => 'No'])] as $fn) {
            denied(403, $fn);
        }$auth->setIdentity($users['alice']);
    });
    test('move persists and stale version is 409 without activity', function () use ($tickets, $a, $id, $boardA, $lane, $current) {
        $before = scalar('SELECT COUNT(*) FROM activities');
        $move = ['version' => 1,'column_id' => $boardA['columns'][1]['id'],'swimlane_id' => $lane] + $current();
        $tickets->move($a, $id, $move);
        check((int)$tickets->ticket($a, $id)['column_id'] === (int)$boardA['columns'][1]['id'], 'move lost');
        denied(409, fn () => $tickets->move($a, $id, $move));
        check((int)scalar('SELECT COUNT(*) FROM activities') === (int)$before + 1, 'rejected move logged');
    });
    test('stale board revision rejected', function () use ($tickets, $a, $data) {
        denied(409, fn () => $tickets->create($a, $data + ['board_revision' => 1]));
    });
    test('nonmember cannot mutate or read ticket', function () use ($auth, $users, $tickets, $query, $a, $id) {
        $auth->setIdentity($users['bob']);
    ...
    test('dense position rebalance preserves unique order', function () use ($pdo, $tickets, $a, $id, $first, $second, $col, $lane, $current) {
        ...
        $tickets->move($a, $id, ['version' => 2,'column_id' => $col,'swimlane_id' => $lane,'placement' => 'between','left_id' => $first,'right_id' => $second] + $current());
        $q = $pdo->prepare('SELECT id FROM tickets WHERE project_id=? ORDER BY position');
        $q->execute([$a]);
        check(array_map('intval', $q->fetchAll(PDO::FETCH_COLUMN)) === [$first,$id,$second], 'wrong order after rebalance');
    });
    test('foreign/nonadjacent neighbors rejected', function () use ($tickets, $a, $id, $col, $lane, $second, $first, $current) {
        denied(409, fn () => $tickets->move($a, $id, ['version' => 3,'column_id' => $col,'swimlane_id' => $lane,'placement' => 'between','left_id' => $second,'right_id' => $first] + $current()));
    });
    test('ORM external transaction remains owned by caller', function () use ($pdo, $tickets, $a, $data, $current) {
        $before = scalar('SELECT COUNT(*) FROM tickets');
    ...
    test('comment authorship and versions', function () use ($auth, $users, $comments, $a, $id) {
        ...
        denied(403, fn () => $comments->save($a, $id, ['id' => $comment,'version' => 1,'body' => 'Hijack']));
        $auth->setIdentity($users['alice']);
        $comments->save($a, $id, ['id' => $comment,'version' => 1,'body' => 'Revised']);
        denied(409, fn () => $comments->save($a, $id, ['id' => $comment,'version' => 1,'body' => 'Lost update']));
        $comments->save($a, $id, ['id' => $comment,'version' => 2,'action' => 'delete']);
        check(scalar('SELECT deleted_at FROM comments WHERE id=?', [$comment]) !== null, 'comment not soft deleted');
    });
    test('last owner and role delegation protected', function () use ($projects, $a, $auth, $users) {
        denied(422, fn () => $projects->member($a, ['email' => 'alice@example.test','role' => 'remove']));
        $projects->member($a, ['email' => 'member@example.test','role' => 'manager']);
        $auth->setIdentity($users['member']);
        denied(403, fn () => $projects->member($a, ['email' => 'viewer@example.test','role' => 'owner']));
        denied(403, fn () => $projects->member($a, ['email' => 'alice@example.test','role' => 'viewer']));
        $auth->setIdentity($users['alice']);
    });
    test('active membership revoked immediately', function () use ($projects, $a, $auth, $users, $query) {
    ...
    test('nonempty column and default lane cannot be removed', function () use ($projects, $a, $col, $lane) {
        denied(422, fn () => $projects->structure($a, ['kind' => 'column','id' => $col,'action' => 'delete']));
        denied(422, fn () => $projects->structure($a, ['kind' => 'swimlane','id' => $lane,'action' => 'delete']));
    });
    test('fulltext and combined filters stay scoped', function () use ($query, $a, $labelB) {
        ...
        check($query->board($a, ['q' => 'sunflower','priority' => 'urgent'])['total'] === 0, 'filters not combined');
    });
    test('archive is read-only and restorable', function () use ($projects, $tickets, $query, $a, $data, $current) {
    ...
    echo json_encode(['driver' => $pdo->getAttribute(PDO::ATTR_DRIVER_NAME),'passed' => count($passed),'tests' => $passed], JSON_PRETTY_PRINT | JSON_THROW_ON_ERROR)."\n";
} catch (Throwable $e) {
