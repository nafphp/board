...
$c                                 = \Naf\app()->container();
$settings                          = $c->get(Config::class)->all();
...
$command = $c->make(QueueConsumeCommand::class);
$once    = fn() => $command->run(
...
foreach ([$started, $finished] as $file) {
    if (is_file($file)) {
        unlink($file);
    }
}
...
try {
    ...
    if (!is_file($started)) {
        throw new RuntimeException('Test worker did not enter its job.');
    }
    proc_terminate($process, 9);
    proc_close($process);
    $process = null;
    if (
        (int) $pdo->query('SELECT COUNT(*) FROM naf_queue_jobs WHERE attempts=1')->fetchColumn()
        !== 1
    ) {
        throw new RuntimeException('Killed worker lost its durable reservation.');
    }
    ...
    if (proc_close($retry) !== 0) {
        throw new RuntimeException('Replacement worker failed.');
    }
    if (
        (int) $pdo->query('SELECT COUNT(*) FROM naf_queue_jobs')->fetchColumn() !== 0
        || file_get_contents($finished) !== "completed\n"
    ) {
        throw new RuntimeException('Replacement did not execute and acknowledge once.');
    }
    $driver->enqueue('MissingNafinityProbeClass', []);
    if ($once() === 0) {
        throw new RuntimeException('Missing class returned success.');
    }
    if (
        (int) $pdo
            ->query("SELECT COUNT(*) FROM naf_queue_jobs WHERE state='failed'")
            ->fetchColumn() !== 1
    ) {
        throw new RuntimeException('Missing class did not deadletter.');
    }
    echo json_encode(
        [
            'passed' => [
                'actual worker SIGKILL preserves reservation',
                'fresh worker reclaims expired lease and acknowledges',
                'missing job class returns failure and enters deadletter',
            ],
            'lease_deadline_advanced_for_test' => true,
        ],
        JSON_PRETTY_PRINT,
    ) . "\n";
} finally {
