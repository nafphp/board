...
for package,constraint in data['require'].items():
    if not package.startswith('naf/'): continue
    name=package.split('/')[1]
    # Explicit development aliases; these are never claimed to be published releases.
    data['repositories'].append({'type':'path','url':'../packages/'+name,'options':{'symlink':True,'versions':{package:constraint.lstrip('^')+'-dev'}}})
